 ...
 * {@link Yx6Format#CHANNELS} however few a source fills: the compact
...
public record Tune(int frames, int frameRate, long masterClock, int loopFrame,
    ...
    public Tune {
        if (registers.length != Yx6Format.REGISTER_STREAMS) {
            throw new IllegalArgumentException("a tune carries "
                    + Yx6Format.REGISTER_STREAMS + " frame streams, R0 to R13, not "
    ...
    private static byte[][] widen(byte[][] streams, int frames) {
        ...
        for (int channel = streams.length; channel < Yx6Format.CHANNELS; channel++) {
