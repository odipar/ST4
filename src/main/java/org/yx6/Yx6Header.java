...
/**
 * The handful of {@code .yx6} header fields the build tools need: what the
 * player must be assembled for, and what the SNDH tags declare.
 *
 * <p>The unit size is not in the yx6 header at all - it lives in the low byte
 ...
 */
public record Yx6Header(int ring, int chunk, int unit, int hz, int flags, int frames) {

    /** Bit 0 of the flags: the tune loops instead of ending. */
    public boolean loops() {
        return (flags & Yx6Format.FLAG_LOOPS) != 0;
    }
    ...
    public static Yx6Header read(Path path) throws IOException {
        byte[] file = Files.readAllBytes(path);
        if (file.length < Yx6Format.HEADER_SIZE
                || word(file, Yx6Format.OFFSET_MAGIC) != (Yx6Format.MAGIC >>> 16)
        ...
        return new Yx6Header(word(file, Yx6Format.OFFSET_RING_SIZE),
                word(file, Yx6Format.OFFSET_CHUNK),
                file[section + 3] & 0xFF,       // the ST4 signature's low byte
                word(file, Yx6Format.OFFSET_PLAYER_HZ),
                word(file, Yx6Format.OFFSET_FLAGS),
                (int) longAt(file, Yx6Format.OFFSET_FRAMES));
    }
