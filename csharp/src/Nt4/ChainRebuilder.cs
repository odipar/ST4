...
internal sealed class ChainRebuilder
{
    ...
    internal ChainRebuilder(int[] units, int literalBits, int[] optimalBits,
    ...
    internal Block Rebuild()
    {
        ...
        var winner = new Block?[units.Length];
        var states = new Dictionary<long, Block>
        {
            [StateKey(Optimizer.InitialOffset, -1)] =
                new Block(-1, -1, Optimizer.InitialOffset, null),
        };
        ...
        Block?[] winner, Stack<Frame> stack)
    {
        ...
        switch (winKind[index])
        {
            case Literals:
            {
                if (!states.TryGetValue(StateKey(offset, winAux[index]), out Block? state))
                {
        ...
        Block?[] winner, Stack<Frame> stack)
    {
        ...
        {                       // the rep candidate exists
            if (!states.TryGetValue(StateKey(offset, frame.PrevEnd), out Block? previous))
            {
        ...
        Block? split = winner[end - frame.NewLength];
        if (split == null)
        {
            stack.Push(new Frame(false, 0, end - frame.NewLength));
            return false;
        }
        states[StateKey(offset, end)] = new Block(frame.NewBits, end, offset, split);
        return true;
    }
    ...
    private int PreviousStateEnd(int offset, int from)
    {
        ...
        for (int index = from; index >= offset; index--)
        {
            if (units[index] == units[index - offset])
            {
                ...
                if (offset == Optimizer.InitialOffset
                    || (index - 1 >= offset
        ...
        return offset == Optimizer.InitialOffset ? -1 : None;
    }
}
