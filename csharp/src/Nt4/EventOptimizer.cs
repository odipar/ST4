...
public sealed class EventOptimizer
{
    ...
    private EventOptimizer(int[] units, int unit, int offsetLimit)
    {
        this.units = units;
    ...
    public static Block Optimize(int[] units, int unit, int offsetLimit, bool progress)
    {
        ...
        var optimizer = new EventOptimizer(units, unit, offsetLimit);
        if (optimizer.CountEvents() > (long)Churn * units.Length)
        {
            return FastOptimizer.Optimize(units, unit, offsetLimit, progress);
        }
