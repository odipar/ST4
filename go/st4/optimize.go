...
func Optimize(units []uint32, unit, offsetLimit int, progress bool) *Block {
	return OptimizePenalty(units, unit, offsetLimit, progress, 0)
}

// OptimizePenalty is Optimize with penalty bits charged on every block
// besides what it writes, so the parse prefers fewer, longer blocks: a
// decoder parses a block at a time, and the bits are not what that costs.
// At a penalty of zero it is Optimize.
func OptimizePenalty(units []uint32, unit, offsetLimit int, progress bool,
	penalty int) *Block {
	literalBits := 8 * unit
	count := len(units)
...
func forward(units []uint32, literalBits, offsetLimit int, optimalBits []int,
	winKind []byte, winOffset, winAux []int, progress bool, penalty int) {
	count := len(units)
		...
		for offset := 1; offset <= maxOffset; offset++ {
			if index != 0 && unitValue == units[index-offset] {
				...
				if litEnd[offset] != none {
					b := litBits[offset] + 1 +
						eliasGammaBits(index-litEnd[offset]) + penalty
					stateBits[offset] = b
...
// an offset kept when it last matched there. Frames form a chain of single
...
// resolveState finds the state an offset had after matching at f.index: the
...
func (r *rebuilder) resolveState(f *frame, states map[int64]*Block,
	...
	if !f.scanned {
		...
		if run >= 2 {
			...
			f.newBits = bestCore + 3 + offsetBits + r.penalty
		}
	}
	...
	if f.prevEnd != none { // the rep candidate exists
		...
		repBits := literal.Bits + 1 + eliasGammaBits(end-f.runStart+1) + r.penalty
...
func (r *rebuilder) literalRun(state *Block, litEnd int) *Block {
	...
	b := state.Bits + 1 + eliasGammaBits(length) + length*r.literalBits + r.penalty
