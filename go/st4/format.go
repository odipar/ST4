...
// the format they share. It is a port of src/main/java/org/st4, written to
// the same bar - one input, one byte stream, out of either.
//
// This is where the port is kept; the trees that pack with it carry a copy,
// and a change here reaches them by copying, so all of them write one byte
// stream for one input.
//
// Stream A has the bits, the flags and the interlaced Elias gamma lengths,
// read a word at a time. Stream B has the literal units, stream C the byte
...
// IsUnitSize reports whether unit is one the format defines.
func IsUnitSize(unit int) bool {
	return unit == 1 || unit == 2 || unit == 4
}

// CheckUnit returns the reason unit cannot be used, or an empty string.
...
// Container is one ST4 file, read apart: the four streams, the unit size,
...
// Read reads a container apart, checking everything a decoder would
