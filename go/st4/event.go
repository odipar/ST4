...
// starts or ends. Every minimum is read over encoded (key, offset) longs, so
...
// sortedKeys returns a group's keys in a fixed order. The result does not
...
func (e *engine) countEvents() int64 {
	...
	// The pass spent the chains; rebuild them empty for the real run.
