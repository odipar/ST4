...
// st4 -c alone writes, read the literals of a full-window parse, fill holes
// of a few units, and shrink the dictionary to what gets copied from. With
...
// reportPass prints a pass's bits and bytes. The line has the time, so
// it draws only where the meter draws: at a terminal, not into a redirected
// log.
func (s *copySearch) reportPass(pass int) {
	if s.progress && isTerminal(os.Stderr) {
		fmt.Fprintf(os.Stderr, "%7.1fs pass %d: %d bits, %d bytes\n",
...
func (s *copySearch) run(deadline time.Time, steps int64) *Block {
	...
	if s.progress && s.step > 0 {
		fmt.Fprintf(os.Stderr, "%d steps, %d accepted: %d bits, %d bytes\n", s.step,
...
func (s *copySearch) report(move string) {
	fmt.Fprintf(os.Stderr, "%7.1fs %8d steps: %d bits, %d bytes  (%s)\n",
...
// visit reads a copy distance whose unit matches at index with the source
