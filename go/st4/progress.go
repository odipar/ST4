...
// or redirected it is a wall of percentages in the reader of it, so it draws
...
// Advance reads one position's worth of steps, and reports when the percent
// moves.
func (m *Meter) Advance(delta int64) {
	...
	fmt.Fprintf(os.Stderr, "\r[%3d%%] %-12s", percent, m.estimate(percent, now))
}
...
func (m *Meter) Finish() {
	...
	if m.enabled {
		fmt.Fprintf(os.Stderr, "\r[100%%] %-12s\n",
