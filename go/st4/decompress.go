...
// reading the stream to its rewind point: from rewindAt bytes on, no match
...
// loop wrongly on the 68000, and is rejected here; NoRewind reads none of it.
...
// copyLength copies length units from the offset the stream keeps: from the
...
// that says the value goes on. The count is kept in 32 bits, as the other two
// trees keep it, so a stream of set bits wraps to a length the caller rejects
