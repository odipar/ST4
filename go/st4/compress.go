...
// PackedSize is the bytes all four streams occupy together.
...
// count: the one copy that would not be leaves its last unit to a literal. The
...
// ran dry - so a set bit patches that byte in place.
