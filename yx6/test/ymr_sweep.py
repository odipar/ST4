...
in the Unicorn rig.
    ...
    python3 yx6/test/ymr_sweep.py song.ymr [more.ymr ...]
...
WHAT IS EXCLUDED, and why. This test sees one thing: the writes YX6_play
...
os.chdir(EMU)
import test_yx6 as T                                                # noqa: E402
...
#   YMR_FRAME_CAP=11000 python3 yx6/test/ymr_sweep.py song.ymr
...
def sweep(path):
    ...
    with tempfile.NamedTemporaryFile(suffix='.yx6', delete=False) as handle:
        yx6 = handle.name
    try:
        ...
        return play(name, dump, open(yx6, 'rb').read(), warns)
...
def play(name, dump, packed, warns):
    ...
    player = T.Player(packed, T.YX6_FIXED + T.STREAMS * ring)
    if player.init() != 0:
        return 'INITFAIL %s' % name
    problem = mfp_problem(player.mfp, dump.named)
    if problem:
        return 'ISSUE %s: YX6_init %s' % (name, problem)
