...
instruction back from a template (`yx6_movep`) rather than from a
hand-encoded opcode.

**Three layout constants.** A write is twelve bytes now rather than ten,
with the ring displacement at +6 and the movep at +8: `YX6_WRITE_READ`,
`YX6_WRITE_MOVEP`, `YX6_WRITE_SIZE`, so the init patch loops and the gate
cannot drift apart.
