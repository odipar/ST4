...
- Writing TDR while stopped loads the main counter immediately; while
  ...
  reason YX6 reloads only on a CHANGED count).
...
| YX6 v1, and v2 as shipped | deterministic: every code arrival is stop -> vector:=loud half -> TDR -> TCR; a held code reloads TDR on change only; a prescaler-slide retunes without touching the vector | loud |
...
it is YX6's own, kept because it is deterministic, hardware-honest, and
it is the sound this player has always made.

## What YX6 v2 does (decided by ear against ym2149-rs)
