...
| ring | The output a streaming decoder keeps, `M` units of it, which the packer is told with `-mN`. | decoders.md, which one |
...
| window | `M`, which see. The word ST4 uses where DTX says ring. | SPEC.md 4.4 |
